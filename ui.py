...
class App(tk.Tk):
    ...
    def show_home(self):
        ...
        if members:
            for mid, name in members:
                ...
                inline_btn(btn_frame, "Open ➔",
                            lambda i=mid, n=name: self.show_lists(i, n),
                            ACCENT, HOVER_ACCENT).pack(side="left", padx=(0, 4))
                inline_btn(btn_frame, "🗑",
        ...
        sep(self.main)
        nav = tk.Frame(self.main, bg=BG); nav.pack()
    ...
    def show_lists(self, mid, mname):
        clear(self.main)
        nav = tk.Frame(self.main, bg=BG); nav.pack(fill="x", pady=(0, 10))
    ...
    def show_list(self, lid, lname, mid, mname, readonly=False):
        clear(self.main)
        nav = tk.Frame(self.main, bg=BG); nav.pack(fill="x")
        back_cmd = self.show_view_pick if readonly else lambda: self.show_lists(mid, mname)
        ghost_btn(nav, "⬅ Back", back_cmd).pack(side="left")

        header_row = tk.Frame(self.main, bg=BG); header_row.pack(fill="x", pady=(10, 0))
        tk.Label(header_row, text=lname, bg=BG, fg=TEXT,
                 font=(FONT_FAM, 22, "bold")).pack(side="left")
        if not readonly:
            tag = tk.Frame(header_row, bg=CARD2, padx=8, pady=4)
            tag.pack(side="right")
        ...
        stats = tk.Frame(self.main, bg=BG); stats.pack(fill="x", pady=12)
        ...
        for iid, mem, iname, date, ib, bought_by, bought_date in items:
            ...
            left = tk.Frame(content, bg=row_bg); left.pack(side="left", fill="x", expand=True)
            tk.Label(left, text=iname, bg=row_bg, fg=MUTED if ib else TEXT,
                     font=(FONT_FAM, 12, "overstrike" if ib else "bold")).pack(anchor="w")

            # Meta row: added by + bought by
            meta = tk.Frame(left, bg=row_bg); meta.pack(anchor="w", pady=(2, 0))
            tk.Label(meta, text=f"Added by {mem} · {date}",
                     bg=row_bg, fg=MUTED, font=(FONT_FAM, 8)).pack(side="left")
            if ib and bought_by:
                tk.Label(meta, text=f"   ·   Bought by {bought_by}",
                         bg=row_bg, fg=ACCENT2, font=(FONT_FAM, 8, "bold")).pack(side="left")

            if not readonly:
                right = tk.Frame(content, bg=row_bg); right.pack(side="right")
    ...
    def _toggle(self, iid, cur, lid, lname, mid, mname):
        db_toggle_bought(iid, cur, mname); self.show_list(lid, lname, mid, mname)
